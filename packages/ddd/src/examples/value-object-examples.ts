...
const EffectEmailSchema = EffectDdd.Schema.String.pipe(
	EffectDdd.Schema.pattern(/^[^\s@]+@[^\s@]+\.[^\s@]+$/),
	EffectDdd.Schema.brand('Email'),
);
...
export async function runValueObjectExamples() {
	...
	const effectEmail = await EffectDdd.Effect.runPromise(EffectEmail.create('test@example.com'));
	const effectSameEmail = await EffectDdd.Effect.runPromise(EffectEmail.create('test@example.com'));
