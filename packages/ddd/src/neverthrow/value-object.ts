...
function makeValueObject<const TSchema extends z4.ZodTypeAny>(schema: TSchema) {
	...
	class CreatedValueObject extends ValueObjectBase<Value> {
		...
		public static create(input: z4.input<TSchema>): Result<CreatedValueObject, ValueObjectValidationError> {
			const parsed = schema.safeParse(input);

			if (!parsed.success) {
				return err(new ValueObjectValidationError('Value object validation failed', parsed.error));
			}

			return ok(new CreatedValueObject(parsed.data));
		}
	}
