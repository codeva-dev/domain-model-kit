...
import { normalizePersistHandlerResult, type IPersistHandler, type PersistHandlerError } from './persist-handler.js';

export type RepositoryPersistHandlers<TContext = unknown> = readonly IPersistHandler<DomainEventBase, TContext, unknown>[];
...
export type SaveError<THandlers extends AnyRepositoryPersistHandlers> = PersistHandlerError<THandlers[number]>;
export type HandlerContext<THandlers extends AnyRepositoryPersistHandlers> =
	THandlers[number] extends IPersistHandler<DomainEventBase, infer TContext, unknown> ? TContext : never;
...
export abstract class RepositoryBase<TDbContext extends TContext, TContext, THandlers extends AnyRepositoryPersistHandlers> {
	 ...
	 */
	public save<TAggregate extends AnyAggregateRoot>(
		aggregate: TAggregate,
		context: TContext = this.dbContext,
	): ResultAsync<void, SaveError<THandlers>> {
		...
		let result = okAsync<void, SaveError<THandlers>>(void 0);
		...
		for (const handler of this.persistHandlers) {
			...
			result = result.andThen(() =>
				normalizePersistHandlerResult(handler.handle(acceptedEvents, context)) as ResultAsync<
					void,
					SaveError<THandlers>
				>,
			);
		...
		return result.andThen(() => {
			...
			return okAsync(void 0);
		}) as ResultAsync<void, SaveError<THandlers>>;
	}
}
