...
> = {
	...
	create(props: { instanceId: DomainInstanceId; payload: unknown; occurredAt?: Date }): Result<TEvent, DomainEventValidationError>;
	decode(serialized: DomainEventDecodeInput): Result<TEvent, DomainEventValidationError>;
...
function parseOccurredAt(occurredAt: string | Date): Result<Date, DomainEventValidationError> {
	const date = occurredAt instanceof Date ? occurredAt : new Date(occurredAt);

	if (Number.isNaN(date.getTime())) {
		return err(new DomainEventValidationError('Domain event occurredAt is invalid'));
	}
...
function makeDomainEvent<const TEventKey extends string, const TPayloadSchema extends z4.ZodTypeAny>(
	...
	class CreatedDomainEvent extends DomainEventBase<TEventKey, Payload> {
		...
		public static create(props: {
			...
			payload: unknown;
			occurredAt?: Date;
		}): Result<CreatedDomainEvent, DomainEventValidationError> {
			const payload = payloadSchema.safeParse(props.payload);

			if (!payload.success) {
				return err(new DomainEventValidationError('Domain event payload validation failed', payload.error));
			}
			...
			return ok(new EventClass({ ...props, payload: payload.data }));
		}

		public static decode(serialized: DomainEventDecodeInput): Result<CreatedDomainEvent, DomainEventValidationError> {
			if (serialized.eventKey !== eventKey) {
				return err(new DomainEventValidationError(`Expected domain event "${eventKey}", got "${serialized.eventKey}"`));
			}

			return parseOccurredAt(serialized.occurredAt).andThen((occurredAt) =>
				this.create({
...
): Result<InstanceType<TEventClass>, DomainEventValidationError> {
	return eventClass.decode(serialized) as Result<InstanceType<TEventClass>, DomainEventValidationError>;
}
...
function makeDomainEventDecoder<const TEventClasses extends readonly DomainEventClass[]>(...eventClasses: TEventClasses) {
	...
	return {
		...
		decode(serialized: DomainEventDecodeInput): Result<Event, DomainEventValidationError> {
			const eventClass = eventsByKey.get(serialized.eventKey);

			if (!eventClass) {
				return err(new DomainEventValidationError(`Unknown domain event "${serialized.eventKey}"`));
			}

			return eventClass.decode(serialized) as Result<Event, DomainEventValidationError>;
		},
	};
}
...
export const DomainEvent = {
	 ...
	 * const eventResult = OrderEventDecoder.decode(serialized)
	 */
