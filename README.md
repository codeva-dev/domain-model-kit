...
```bash
npm install github:codeva-dev/domain-model-kit#<tag-or-commit>
```
...
- [`examples/src/order-pure.ts`](./examples/src/order-pure.ts)
- [`examples/src/order-neverthrow.ts`](./examples/src/order-neverthrow.ts)
- [`examples/src/order-effect.ts`](./examples/src/order-effect.ts)
- [`examples/src/value-object-examples.ts`](./examples/src/value-object-examples.ts)
...
This repository is still an npm workspace monorepo:

```text
./              Package root: @codeva-dev/domain-model-kit
examples/       Private TypeScript workspace with runnable examples
```

The package root is the repository root. That keeps npm publishing unchanged while also making GitHub installs work from a tag or commit because `prepare` builds `dist` during git dependency installation.

The examples workspace depends on the root package through `file:..` and imports only the public subpaths. It is intentionally private and is not part of the published package tarball.
